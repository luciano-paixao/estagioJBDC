    ...
    private void salvarConcedenteCompleta() {
        ...
        for (PainelTelefone painelTel : view.getListaPaineisTelefone()) {
            ...
            if (!numero.trim().isEmpty()) {
                telefonesValidos++;
                // Telefone tel = new Telefone(ddd, numero, painelTel.getTipoSelecionado());
                // listaTelefonesModel.add(tel);
            }
        }

        String nomeRep = view.getPainelRepresentante().getNome();

        // Lógica de Modelo e Service virá aqui...
        // concedenteService.salvar(concedente, listaTelefonesModel, endereco, representante);
